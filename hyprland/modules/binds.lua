...
local menu        = "rofi -modi run,drun,window,keys -show drun -show-icons -show keys -sidebar-mode"
...
for i = 1, 9 do
    local key = "code:" .. (i + 9) -- rangée du haut, indépendant du layout

    -- Aller au workspace i
    hl.bind(mainMod .. " + " .. key,
        hl.dsp.focus({ workspace = tostring(i) }))

    -- Déplacer le client focus vers le workspace i (sans le suivre)
    hl.bind(mainMod .. " + SHIFT + " .. key,
        hl.dsp.window.move({ workspace = tostring(i), follow = false }))

    -- Déplacer ET suivre le client vers le workspace i
